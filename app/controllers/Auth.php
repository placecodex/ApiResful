...
// check password match
      ...
      # return  ejson_encode(array('status' => 'password not match'));
        ...
        if (empty($username) || empty($password) ||  empty($repassword) || empty($full_name)  ||  strlen($username) > 20 || strlen($password) > 20 || 
        ...
        $query = $this->db->query("SELECT username, email FROM $this->userTbl WHERE 'username'='{$username}' ||  email='{$email}' ");
        if($query->num_rows() >= 1){
           ...
           $insert = $this->db->insert($this->userTbl, $data); 
            ...
            if ($insert) {
            	$query = $this->db->query("SELECT user_id FROM $this->userTbl WHERE email='{$email}'");
