...
//instanciamos al objeto codeigniter 
	$CI =& get_instance(); // cargamos la base de datos 
	$CI->load->database('default'); 
	// obtenemos el valor del item 'sess_use_database' 
	$sess_use_database = $CI->config->item('database');
	...
	if (!isset($arrSesion['usuario'])){ return false;
	// indicamos no is_logged 
	}else{ 
		...
		// consultamos por id 
			$CI->db->from('ci_sesion'); $CI->db->where('id',$session_id); $query = $CI->db->get(); 
			// si coincide el session_id en algun registro 
			// es porque el usuario tiene sesion abierta
			 return ($query->num_rows()>0) ? true : false; }else{ // return 
			 	(!empty($session_id)) ? true : false; } } $CI->db->close(); unset($CI); } } 
		...
		// si se esta usando la base de datos para las sesiones 
			 		...
			 		// indicara si se elimino la seison de la db 
			 			...
			 			$CI->db->select('data,id'); $CI->db->from('ci_sesion'); $query = $CI->db->get(); $arrDatos = array(); 
			// recorremos la lista de usuarios con sesion en la db 
			 			foreach($query->result() as $row){ 
			 			// obtenemos el user_data de la fila 
			 			$valor = $row->user_data; 
			 			// los datos estan serializados en la db // asi que los deserializamos 
			 			...
			 			// borramos la sesion de la db 
			 				$CI->db->delete('ci_sesion',array('id' => $row->session_id)); 
			 				echo $CI->db->last_query();exit; $delete = true; break; } } if($delete){ 
			 				// cerramos la sesion 
			 					$CI->session->sess_destroy(); } }else{ 
			 					// cerramos la sesión 
			 			...
			 			$CI->db->where('last_activity <',(time() - 3600));
			 			//3600 - 1 hora 
			 			$CI->db->delete('ci_sesion'); $CI->db->close(); 
#-------------------------------------------------------------------
			 			unset($CI); 
